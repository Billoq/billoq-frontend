...
export function OverviewChart({
  ...
  // Helper date functions - now using useCallback
  ...
  const subDays = useCallback((date: Date, days: number): Date => addDays(date, -days), [addDays]);
  const subWeeks = useCallback((date: Date, weeks: number): Date => addDays(date, -weeks * 7), [addDays]);
  const subMonths = useCallback((date: Date, months: number): Date => addMonths(date, -months), [addMonths]);
