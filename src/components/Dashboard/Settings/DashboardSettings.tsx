...
const supportedChainDefinitions = isMainnet ? mainnetChains : testnetChains;
...
export default function DashboardSettings() {
  ...
  const chain = activeChain ?? defaultChain;
  ...
  const handleSwitchChain = async (targetChainId: number) => {
    const target = supportedChainDefinitions.find((c) => c.id === targetChainId);
    if (!target) return;

    try {
      await switchChain(target);
      const chainColors = getChainColor(target.id);
      toast.success(`Switched to ${target.name} network`, {
        position: "bottom-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
              ...
              <Card className="border-[#1E293B] bg-[#1A202885]">
                ...
                <CardContent className="space-y-4 md:space-y-6 px-4 md:px-6">
                  {isConnected ? (
                    <>
                      <div
                        className={`bg-[#111C2F] border rounded-lg p-4 md:p-6 relative overflow-hidden ${currentChainColors.border}`}
                      >
                        <div
                          className="absolute -top-20 -right-20 w-40 h-40 opacity-10 rounded-full blur-3xl"
                          style={{ backgroundColor: currentChainColors.hex }}
                        />

                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative">
                          <div className="flex items-center gap-3 md:gap-4">
                            <div className="relative">
                              <Wallet className="w-8 h-8 text-[#1B89A4]" />
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <Badge
                                  className={`${currentChainColors.bg} ${currentChainColors.text} ${currentChainColors.border} text-xs border`}
                                >
                                  Connected
                                </Badge>
                                {!isMainnet && (
                      ...
                      <div className="space-y-3 md:space-y-4">
                        ...
                        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-3 w-full">
                          {supportedChainDefinitions.map((network) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {isActive && (
                                  <Check
                                    className={`ml-auto h-3 w-3 md:h-4 md:w-4 ${networkColors.text} flex-shrink-0`}
                                  />
                                )}
                              </button>
                            );
                        ...
                        <div
                          className={`mt-4 p-3 bg-[#111C2F] border rounded-lg ${currentChainColors.border}`}
                        >
                          <div className="flex items-center gap-2 text-sm text-[#94A3B8]">
                            ...
                            <span>
                              ...
                              configuration on{" "}
                              <strong className={currentChainColors.text}>{chain.name}</strong>
                            </span>
                          </div>
